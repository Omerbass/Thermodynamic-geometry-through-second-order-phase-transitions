...
First, we briefly define the key metrics and scaling tools used in our study.
...
We are using the dissipation metric, first introduced in \cite{sivakThermodynamicMetricsOptimal2012}. It is defined as
\begin{equation}
    g_{\mu\nu}^S = \int_0^\infty \langle \delta X_\mu(t)\, \delta X_\nu(0) \rangle\, dt =
    \mathcal{T}_{\mu\nu} \frac{\partial^2 \ln Z}{\partial \lambda^\mu \partial \lambda^\nu}
\end{equation}
where $\mathcal{T}_{\mu\nu}$ is the time-response matrix, $Z$ the partition function, and $X_\mu$ the conjugate variable to $\lambda^\mu$ -- the natural variables of the free energy (e.g., $\{S, M\}$ for $F(S, M)$).
The response time is directly encoded in the metric through $\mathcal{T}_{\mu\nu}$, and the dissipated availability can be expressed as
\begin{equation}
    \mathcal{A} = \int_{\boldsymbol{\lambda}(t)} g^S_{\mu\nu}\, \dot{\lambda}^\mu \dot{\lambda}^\nu\, dt,
\end{equation}
which is subject to the inequality
\begin{equation}\label{Eq:A_bound}
    \mathcal{A} \ge \frac{\mathcal{L}^2}{\tau}
\end{equation}
 saturated by constant-speed protocols.

%\subsection{Scaling Hypothesis} \label{subsec:ScalingHypothesisFramework}
Near second order phase transitions, free energy derivatives--- and hence the dissipation metric--- often diverge. Although exact partition functions are rarely tractable, the asymptotic form of these divergences is determined by critical exponents defining universality classes.

The scaling hypothesis relates the free energy $\varphi$ near the critical point $(T_c, H_c)$ to reduced variables $t = (T - T_c)/T_c$ and $h = (H - H_c)/H_c$:
...
The exponents $b$, $\Delta$, are directly related to the standard critical exponents through the hyperscaling relations, e.g., $b = 2 - \alpha$ from the heat capacity exponent $\alpha$.
This approach captures metric divergences across universality classes but does not address relaxation times, which are critical for the dissipation metric. The dynamic scaling hypothesis relates the relaxation time $\tau$ and correlation length $\xi$ as
\begin{equation}
    \tau \propto \xi^z
\end{equation}
...
A diverging metric does not necessarily imply a divergent thermodynamic length. For constant-speed protocols of fixed duration, Eq.(\ref{Eq:A_bound}) implies $\mathcal{A} \propto \mathcal{L}^2$.
Thus, a finite thermodynamic length guarantees the existence of a finite-dissipation protocol. In addition, from the Cauchy-Schwarz inequality $\mathcal{L} \to \infty$ implies $\mathcal{A} \to \infty$.
