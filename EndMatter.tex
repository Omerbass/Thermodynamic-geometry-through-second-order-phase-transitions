...
\section{Calculating the dissipation metric}
\begin{comment}
...
From these, we calculate the time integrated correlations by incorporate linearized relaxation. As these calculations are easier in $m_i$ variables, we then convert to the $\boldsymbol{\lambda}=(\beta,\beta h) \equiv (\beta, \alpha)$ representation, in which the metric is provided. We set $\alpha\equiv \beta h$ for brevity.
...
In the disordered phase, the magnetizations of the two sublattices are identical. Therefore, the total magnetization
\begin{align}
